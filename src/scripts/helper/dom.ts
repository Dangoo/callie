 ...
 */
function extendNode(node: HTMLElement, data: ElementData) {
  if (data["children"]) {
    node.innerHTML = data["children"];
  }

  if (data.className) {
    data.className.forEach((className) => node.classList.add(className));
  }
 ...
 * @returns        Node
 */
export function renderInNode(target: Node, node: Node) {
 ...
 * @returns              Node
 */
function buildTableRow(cells: ElementData[], cellNodeName: "th" | "td") {
  const row = document.createElement("tr");
  const cell = document.createElement(cellNodeName);

  cells.forEach((value) => {
    const cellInstance = cell.cloneNode(false) as
...
export function buildTable(
  headData: ElementData[] | null,
  bodyData: ElementData[][],
  footData?: ElementData[]
) {
 ...
 */
export function buildList(listNodeName: string, listItemData: ElementData[]) {
  const listNode = document.createElement(listNodeName);
  const listItemNode = document.createElement("li");

  listItemData.forEach((value) => {
    const listItemInstance = listItemNode.cloneNode(false) as HTMLLIElement;
